    ...
    <AppStateContext.Provider value={state}>
      <AppDispatchContext.Provider value={dispatch}>
        <DogStateContext.Provider
          value={React.useMemo(() => [dogState, dogDispatch], [dogState])}
        >
          {children}
        </DogStateContext.Provider>
...
function useDogState() {
  ...
  if (!context) {
    throw new Error('useDogState must be used within the AppProvider')
  }
  return context
}
...
function withStateSlice(Component) {
  return function WithStateSlicer({row, column}) {
    const state = useAppState()
    const cell = state.grid[row][column]
    return <Component cell={cell} row={row} column={column}></Component>
  }
}
...
CellImpl = withStateSlice(CellImpl)
